...
<div class="container">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('warehouse.inventory.update', $item) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('quantity') is-invalid @enderror" 
                           id="quantity" name="quantity" value="{{ old('quantity', $item->quantity) }}" required min="0">
                    @error('quantity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('unit_price') is-invalid @enderror" 
                           id="unit_price" name="unit_price" value="{{ old('unit_price', $item->unit_price) }}" required min="0">
                    @error('unit_price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('package_volume') is-invalid @enderror" 
                           id="package_volume" name="package_volume" value="{{ old('package_volume', $item->package_volume) }}" required min="0">
